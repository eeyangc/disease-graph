# Latent Bivariate Probit Model
library(pbivnorm)

Lbiprobit <- function(Pvalue, X){
  ...
  # Stage 2
  cat("Stage 2 starts... \n", " First GWAS ")
  result2.1 <- Update_beta(Pvalue[, 1], X, result1.1$alpha, 
                           beta0 = c(-qnorm(1 - result1.1$pi1), numeric(K)))
  cat("iterations=", result2.1$iter.times, "\n")
  cat(" Second GWAS ")
  result2.2 <- Update_beta(Pvalue[, 2], X, result1.2$alpha, 
                           beta0 = c(-qnorm(1 - result1.2$pi1), numeric(K)))
...
Update_beta <- function(Pvalue, X, alpha, beta0, maxiter = 1e4, tol = 1e-6){
  ...
  for (iter in 1:maxiter){
    ...
    gamma <- XTX_1 %*% (t(X) %*% E_Z) 
    Xgamma <- X%*%gamma
    sigma2 <- mean(E_Z2 - 2*E_Z*Xgamma + Xgamma^2)
...
Update_all <- function(Pvalue, X, alpha, beta, rho, maxiter = 1e4, tol = 1e-6){
  ...
  for (iter in 1:maxiter){
    ...
    phi <- dnorm(Xbeta)
    red <- phi[, 1]*pnorm((Xbeta[, 2] - rho*Xbeta[, 1])*c)
    green <- phi[, 2]*pnorm((Xbeta[, 1] - rho*Xbeta[, 2])*c)
    
    comp1 <- red + rho*green
    comp2 <- green + rho*red
    comp3 <- -Xbeta[, 1]*red - rho^2*Xbeta[, 2]*green + 
      rho/c*phi[, 2]*dnorm((Xbeta[, 1] - rho*Xbeta[, 2])*c)
    comp4 <- -Xbeta[, 2]*green - rho^2*Xbeta[, 1]*red + 
      rho/c*phi[, 1]*dnorm((Xbeta[, 2] - rho*Xbeta[, 1])*c)
    comp5 <- -rho*Xbeta[, 1]*red - rho*Xbeta[, 2]*green +
      phi[, 1]/c*dnorm((Xbeta[, 2] - rho*Xbeta[, 1])*c)
    
    Xbeta_phi <- Xbeta*phi
    
    E_Z1 <- Xbeta[, 1] + pi11*comp1/(Phi11 + (Phi11 == 0)) + 
      pi10*(phi[, 1] - comp1)/(Phi10 + (Phi10 == 0)) +
      pi01*(rho*phi[, 2] - comp1)/(Phi01 + (Phi01 == 0)) + 
      pi00*(-phi[, 1] - rho*phi[, 2] + comp1)/(Phi00 + (Phi00 == 0))
    E_Z2 <- Xbeta[, 2] + pi11*comp2/(Phi11 + (Phi11 == 0)) + 
      pi10*(rho*phi[, 1] - comp2)/(Phi10 + (Phi10 == 0)) +
      pi01*(phi[, 2] - comp2)/(Phi01 + (Phi01 == 0)) + 
      pi00*(-phi[, 2] - rho*phi[, 1] + comp2)/(Phi00 + (Phi00 == 0))
    E_Z <- cbind(E_Z1, E_Z2)
    E_Z1_2 <- Xbeta[, 1]*(2*E_Z1 - Xbeta[, 1]) + 1 +
      pi11*comp3/(Phi11 + (Phi11 == 0)) + 
      pi10*(-Xbeta_phi[, 1] - comp3)/(Phi10 + (Phi10 == 0)) +
      pi01*(-rho^2*Xbeta_phi[, 2] - comp3)/(Phi01 + (Phi01 == 0)) +
      pi00*(Xbeta_phi[, 1] + rho^2*Xbeta_phi[, 2] + comp3)/(Phi00 + (Phi00 == 0))
    E_Z2_2 <- Xbeta[, 2]*(2*E_Z2 - Xbeta[, 2]) + 1 +
      pi11*comp4/(Phi11 + (Phi11 == 0)) + 
      pi10*(-rho^2*Xbeta_phi[, 1] - comp4)/(Phi10 + (Phi10 == 0)) +
      pi01*(-Xbeta_phi[, 2] - comp4)/(Phi01 + (Phi01 == 0)) +
      pi00*(Xbeta_phi[, 2] + rho^2*Xbeta_phi[, 1] + comp4)/(Phi00 + (Phi00 == 0))
    E_Z1_Z2 <- Xbeta[, 1]*Xbeta[, 2] + Xbeta[, 1]*(E_Z2 - Xbeta[, 2]) + 
      Xbeta[, 2]*(E_Z1 - Xbeta[, 1]) + rho + pi11*comp5/(Phi11 + (Phi11 == 0)) +
      pi10*(-rho*Xbeta_phi[, 1] - comp5)/(Phi10 + (Phi10 == 0)) +
      pi01*(-rho*Xbeta_phi[, 2] - comp5)/(Phi01 + (Phi01 == 0)) +
      pi00*(rho*(Xbeta_phi[, 1] + Xbeta_phi[, 2]) + comp5)/(Phi00 + (Phi00 == 0))
